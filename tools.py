...
def is_withinAgentFiles(path: pathlib.Path) -> bool:
    try:
...
def token_escape_agentFiles(token: str) -> bool:
    ...
    if token.startswith("/"):
        return not is_withinAgentFiles(pathlib.Path(token).resolve(strict=False))
    ...
    if "=" in token:
        ...
        if value != token:
            return token_escape_agentFiles(value)

    return False


def validate_shellCommandScope(command: str) -> tuple[bool, str]:
    try:
    ...
    for token in tokens:
        if token_escape_agentFiles(token):
            return False, (
...
def run_shell(command: str, timeout: int = 30, safe_mode: bool = True, **kwargs) -> str:
    ...
    allowed, reason = validate_shellCommandScope(command)
    if not allowed:
...
def write_file(
    ...
    **kwargs
    ...
    try:
        p = resolve_agentPath(path, allow_missing=True)
